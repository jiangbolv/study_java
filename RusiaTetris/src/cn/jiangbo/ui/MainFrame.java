...
import cn.jiangbo.object.imp1.PieceCreatorImp1;
...
public class MainFrame  extends JFrame {
    ...
    private PieceCreator creator = new PieceCreatorImp1();
    ...
    public void change() {
        ...
        //判断转换后左边是否越界，得到当前最小方块的x坐标
        int minX = this.currentPiece.getMinXLocation();
        if (minX < 0) {
            //右移超过的部分
            this.currentPiece.setSquareXLocation(-minX);
        ...
        if (maxX > gamePanelWidth) {
            //左移超过GamePanel宽的部分
            this.currentPiece.setSquareXLocation(-(maxX - gamePanelWidth));
        }
    ...
    public void right(int size) {
        ...
        this.currentPiece.setSquareXLocation(distance);
        this.gamePanel.repaint();
    }

    //左，参数为距离一格
    private void left(int size) {
        if (this.pauseFlag) return;
        if (this.currentPiece == null) return;
        //判断左边是否有Square
    ...
    private void finishDown() {
        ...
        //设置新的Piece坐标
        this.currentPiece.setSquareXLocation(-NEXT_X);
        this.currentPiece.setSquareXLocation(BEGIN_X);
